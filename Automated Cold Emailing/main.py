...
from utils import (
    preprocess_csv,
    extract_lead_info_from_csv,
    eliminate_incomplete_leads,
    clean_emails,
    add_opt_out_message,
    email_verifier,
    transfer_data_between_files,
)
...
def setup_parser():
    ...
    preprocess_parser = subparsers.add_parser(
        "preprocess", help="Preprocess a CSV file"
    )
    preprocess_parser.add_argument("csv_file", help="Path to the CSV file")
    preprocess_parser.add_argument(
        "columns", nargs="+", help="Column names to preprocess"
    )

    # Extract lead info
    extract_parser = subparsers.add_parser(
        "extract", help="Extract lead info from CSV to Excel"
    )
    ...
    eliminate_parser = subparsers.add_parser(
        "eliminate", help="Eliminate incomplete leads"
    )
    ...
    generate_parser = subparsers.add_parser(
        "generate", help="Generate emails for leads"
    )
    ...
    generate_parser.add_argument(
        "--row_start", type=int, default=2, help="Starting row (default: 2)"
    )
    ...
    verify_parser.add_argument(
        "csv_file", help="Path to the CSV file with verified emails"
    )
    ...
    verify_parser.add_argument(
        "--excel_col", default="Email", help="Excel email column"
    )
    ...
    send_parser.add_argument(
        "--email_limit", type=int, default=50, help="Email limit per sender"
    )
...
def load_templates(template_file):
    ...
    try:
        ...
        if len(sections) < 2:
            raise ValueError(
                "Template file must contain initial email and at least one follow-up separated by '---'"
            )
...
def main():
    ...
    if args.command == "preprocess":
        ...
        clean_emails(
            args.excel_file, args.sheet_name, args.columns, args.row_start, args.row_end
        )
